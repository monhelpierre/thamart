...
export const POST = withAuth(async (req, decodedToken) => {
    ...
    if (!name?.pt || !name?.fr || !name?.en) return NextResponse.json({ error: "name required in all 3 languages" }, { status: 400 });
    if (!description?.pt || !description?.fr || !description?.en) return NextResponse.json({ error: "description required in all 3 languages" }, { status: 400 });
    if (!price || typeof price !== "number" || price <= 0) return NextResponse.json({ error: "invalid price" }, { status: 400 });
    if (!image || typeof image !== "string") return NextResponse.json({ error: "image URL required" }, { status: 400 });
...
export const PATCH = withAuth(async (req, decodedToken) => {
    ...
    if (updates.customizable !== undefined) allowed.customizable = updates.customizable;
